...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: `${s.title} | BuildEstimatory`,
    description: s.metaDescription,
    url: `${siteConfig.url}/guides/16-vs-24-on-center-spacing/`,
  ...
  alternates: {
    canonical: `${siteConfig.url}/guides/16-vs-24-on-center-spacing/`,
    languages: {
      en: "https://buildestimatory.com/guides/16-vs-24-on-center-spacing/",
      es: "https://es.buildestimatory.com/guides/16-vs-24-on-center-spacing/",
      "x-default": "https://buildestimatory.com/guides/16-vs-24-on-center-spacing/",
    },
  },
};
...
export default function SpacingGuidePage() {
  const breadcrumbJsonLd = breadcrumbSchema([
    ...
    { name: s.title, url: "/guides/16-vs-24-on-center-spacing/" },
  ]);
  ...
  const articleJsonLd = {
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `${siteConfig.url}/guides/16-vs-24-on-center-spacing/`,
    },
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <section className="mb-10">
        ...
        <div className="prose prose-gray max-w-none space-y-4">
          <p className="text-gray-600 leading-relaxed">{s.whatIsText1}</p>
          <p className="text-gray-600 leading-relaxed">{s.whatIsText2}</p>
          <p className="text-gray-600 leading-relaxed">{s.whatIsText3}</p>
        </div>
      </section>
      ...
      <section className="mb-10">
        ...
        <p className="text-gray-500 mb-6">{s.comparisonSubheading}</p>

        <div className="overflow-x-auto rounded-xl border border-gray-200">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50">
                <th className="text-left px-4 py-3 font-bold text-gray-900 border-b border-gray-200 w-1/4">
                  {s.comparisonCol1}
                </th>
                <th className="text-left px-4 py-3 font-bold text-orange-600 border-b border-gray-200 w-[37.5%]">
                  {s.comparisonCol2}
                </th>
                <th className="text-left px-4 py-3 font-bold text-gray-700 border-b border-gray-200 w-[37.5%]">
                  {s.comparisonCol3}
                </th>
              </tr>
            </thead>
      ...
      <section className="mb-10">
        ...
        <p className="text-gray-600 mb-4">{s.when16Intro}</p>
        <ul className="space-y-3 mb-4">
          {s.when16Items.map((item, i) => (
      ...
      <section className="mb-10">
        ...
        <p className="text-gray-600 mb-4">{s.when24Intro}</p>
        <ul className="space-y-3 mb-4">
          {s.when24Items.map((item, i) => (
            <li key={i} className="flex gap-3">
              <span className="text-orange-500 font-bold mt-0.5 shrink-0" aria-hidden="true">
                &bull;
              </span>
      ...
      <section className="mb-10">
        ...
        <p className="text-gray-600 mb-4">{s.codeIntro}</p>
        <ul className="space-y-3 mb-4">
          {s.codeItems.map((item, i) => (
            <li key={i} className="flex gap-3">
              <span className="text-orange-500 font-bold mt-0.5 shrink-0" aria-hidden="true">
                &bull;
              </span>
      ...
      <section className="mb-10">
        ...
        <p className="text-gray-600 mb-6">{s.impactIntro}</p>

        <div className="space-y-6">
          <div className="bg-white rounded-lg border border-gray-200 p-5">
            <h3 className="font-bold text-gray-900 mb-2">
              {s.impactInsulationHeading}
            </h3>
            <p className="text-gray-600 leading-relaxed text-sm">
              {s.impactInsulationText}
            </p>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-5">
            <h3 className="font-bold text-gray-900 mb-2">
              {s.impactDrywallHeading}
            </h3>
            <p className="text-gray-600 leading-relaxed text-sm">
              {s.impactDrywallText}
            </p>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-5">
            <h3 className="font-bold text-gray-900 mb-2">
              {s.impactSheathingHeading}
            </h3>
            <p className="text-gray-600 leading-relaxed text-sm">
              {s.impactSheathingText}
            </p>
          </div>
        </div>
      </section>
